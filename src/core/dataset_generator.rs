...
pub fn generate_massive_dataset() -> Result<String, Box<dyn std::error::Error>> {
    ...
    let output = Command::new("python3").arg(python_script).output()?;
...
pub fn load_dataset(path: &str) -> Result<Dataset, Box<dyn std::error::Error>> {
    ...
    println!(
        "✅ [Rust] Loaded {} total samples",
        dataset.metadata.total_samples
    );
    println!("   • Fake news: {}", dataset.metadata.breakdown.fake_news);
    println!(
        "   • Code samples: {}",
        dataset.metadata.breakdown.code_samples
    );
    println!(
        "   • Configurations: {}",
        dataset.metadata.breakdown.configurations
    );
    println!(
        "   • Search data: {}",
        dataset.metadata.breakdown.information_search
    );
...
pub fn prepare_for_chapel_training(
    dataset: &Dataset,
    ...
    assert!(
        !dataset.information_search.is_empty(),
        "Missing search samples"
    );
