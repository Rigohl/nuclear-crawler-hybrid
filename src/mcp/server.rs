...
//! 
//! Exposes EXACTLY 5 TOOLS via HTTP endpoints - NO MOCKS
//! Compatible with Copilot CLI and Claude Desktop
//! 
//! TOOLS: websearch, premium, file_search, scan, info

use crate::mcp::protocol::{MCPRequest, MCPResponse, get_tool_definitions, error_codes};
use crate::mcp::tools::{
    WebSearchTool, PremiumContentTool, AdvancedFileSearchTool, 
    ScanWorkspaceTool, ScanConfig,
};
use crate::{
    cache::Cache, intelligent_storage::IntelligentStorage,
...
/// MCP Server state - PODER REAL
pub struct MCPServer {
    // 🔥 5 HERRAMIENTAS MCP - SIN MOCKS
...
impl MCPServer {
    /// Create new MCP server with REAL implementations
    pub fn new() -> Self {
        eprintln!("🔥 Initializing MCP Server with 5 REAL tools...");
        ...
        eprintln!("🔧 Tools: websearch, premium, file_search, scan, info");
...
async fn handle_tool_call(
    ...
    match tool_name.as_deref() {
        Some("websearch") => {
            execute_websearch(&server, id, arguments).await
        }
        Some("premium") => {
            execute_premium(&server, id, arguments).await
        }
        Some("file_search") => {
            execute_file_search(&server, id, arguments).await
        }
        Some("scan") => {
            execute_scan(&server, id, arguments).await
        }
        Some("info") => {
            execute_info(&server, id, arguments).await
        }
        Some("ai_dataset_trainer") => {
            execute_ai_dataset_trainer(&server, id, arguments).await
        }
        Some(unknown) => {
            Json(MCPResponse::method_not_found(id, unknown))
        }
        None => {
            Json(MCPResponse::invalid_params(id, "tool name required"))
        }
    }
}
...
async fn handle_rpc(
    ...
    match req.method.as_str() {
        ...
        "initialize" => {
            ...
            Json(MCPResponse::success(id, json!({
        ...
        method => {
            Json(MCPResponse::method_not_found(id, method))
        }
    }
}
...
async fn execute_websearch(
    ...
    let deep_search = args.get("deep_search")
    ...
    eprintln!("🔍 WebSearch: '{}' (max: {}, deep: {}, lang: {})", query, max_results, deep_search, language);
...
async fn execute_premium(
    ...
    let input = match args.get("input").and_then(|v| v.as_str()) {
        ...
        _ => return Json(MCPResponse::invalid_params(id, "input parameter required (URL or search phrase)")),
    };
    ...
    let full_content = args.get("full_content")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
        
    let bypass_paywall = args.get("bypass_paywall")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    ...
    eprintln!("📚 Premium: '{}' (type: {}, url: {}, bypass: {})", input, content_type, is_url, bypass_paywall);
    ...
    if is_url {
        // Fetch from URL
        match server.premium_content.fetch_url_real(input, bypass_paywall).await {
            Ok(content) => {
                Json(MCPResponse::success(id, json!({
                    "input": input,
                    "input_type": "url",
        ...
        // Search for content
        match server.premium_content.search_premium_real(input, content_type).await {
            Ok(results) => {
                Json(MCPResponse::success(id, json!({
                    "input": input,
                    "input_type": "search",
...
async fn execute_file_search(
    ...
    let find_errors = args.get("find_errors").and_then(|v| v.as_bool()).unwrap_or(true);
    let find_warnings = args.get("find_warnings").and_then(|v| v.as_bool()).unwrap_or(true);
    let find_todos = args.get("find_todos").and_then(|v| v.as_bool()).unwrap_or(true);
    let context_lines = args.get("context_lines").and_then(|v| v.as_u64()).unwrap_or(2) as usize;
    
    eprintln!("📄 FileSearch: '{}' (query: {:?}, errors: {}, todos: {})", path, query, find_errors, find_todos);
    
    // Apply rate limiting
    server.rate_limiter.wait().await;
    
    // Execute REAL file search
    match server.file_search.search_files_real(
        path, 
        query, 
        is_regex, 
        case_sensitive,
...
async fn execute_scan(
    ...
    let _detect_mocks = args.get("detect_mocks").and_then(|v| v.as_bool()).unwrap_or(true);
    let _detect_todos = args.get("detect_todos").and_then(|v| v.as_bool()).unwrap_or(true);
    let _detect_security = args.get("detect_security").and_then(|v| v.as_bool()).unwrap_or(true);
    let _detect_performance = args.get("detect_performance").and_then(|v| v.as_bool()).unwrap_or(true);
    let _max_depth = args.get("max_depth").and_then(|v| v.as_u64()).unwrap_or(50) as usize;
    let report_format = args.get("report_format").and_then(|v| v.as_str()).unwrap_or("detailed");
...
async fn execute_info(
    ...
    let target = args.get("target").and_then(|v| v.as_str()).unwrap_or("project");
    let path = args.get("path").and_then(|v| v.as_str());
    let include_stats = args.get("include_stats").and_then(|v| v.as_bool()).unwrap_or(true);
    let include_health = args.get("include_health").and_then(|v| v.as_bool()).unwrap_or(true);
    let include_advice = args.get("include_advice").and_then(|v| v.as_bool()).unwrap_or(true);
    
    eprintln!("ℹ️ Info: target='{}' (stats: {}, health: {}, advice: {})", target, include_stats, include_health, include_advice);
    ...
    match target {
        ...
        "file" | "module" => {
            if let Some(p) = path {
                ...
                return Json(MCPResponse::invalid_params(id, "path required for file/module target"));
            }
        }
        "dependencies" => {
            // Analyze Cargo.toml
        ...
        _ => {
            return Json(MCPResponse::invalid_params(id, "invalid target (use: project, file, module, dependencies)"));
        }
    }
...
async fn execute_ai_dataset_trainer(
    ...
    let dataset_name = args.get("dataset_name")
        .and_then(|v| v.as_str())
        .unwrap_or("training_dataset");
    
    let target_size = args.get("target_size")
        .and_then(|v| v.as_u64())
        .unwrap_or(10000) as usize;
    
    let embedding_dim = args.get("embedding_dim")
        .and_then(|v| v.as_u64())
        .unwrap_or(384) as usize;
    
    let export_path = args.get("export_path")
        .and_then(|v| v.as_str())
        .unwrap_or("./dataset_output.jsonl");
    
    let use_go = args.get("use_go_parallel")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let use_zig = args.get("use_zig_dedup")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let use_nim = args.get("use_nim_parsing")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let use_jax = args.get("use_jax_gpu")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    ...
    let notes = vec![
        format!("🔗 Go processor will fetch up to {} sources in parallel", target_size),
        format!("⚡ Zig SIMD will hash each document for efficient deduplication"),
        format!("📄 Nim will extract clean text from HTML"),
        format!("🧠 JAX will generate {}-dimensional embeddings", embedding_dim),
    ];
