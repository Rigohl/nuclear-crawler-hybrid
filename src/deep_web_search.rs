...
impl DeepWebSearch {
    ...
    pub fn new(scraper: Arc<NuclearScraper>, web_search: Arc<WebSearch>) -> Self {
        ...
        let bypass = Arc::new(NuclearBypass::new(bypass_config).unwrap_or_else(|_| {
            NuclearBypass::new(NuclearBypassConfig::default()).unwrap()
        }));
        
        Self { scraper, web_search, bypass }
    }
    ...
    pub async fn search(&self, config: DeepWebSearchConfig) -> Result<Vec<DeepWebSearchResult>> {
        ...
        for crawl_result in crawl_results {
            if crawl_result.status_code == 200 && !crawl_result.html.is_empty() {
                let result = self.process_deep_web_result(
                    &crawl_result,
                    &config,
                ).await?;
    ...
    async fn prepare_deep_web_urls(
        &self,
            ...
            match source {
                DeepWebSource::Academic => {
                    ...
                    for start in (10..=100).step_by(10) {
                        urls.push(format!("https://scholar.google.com/scholar?q={}&start={}", query_encoded, start));
                    }
                    
                    // arXiv (gratis, preprints)
                    urls.push(format!("https://arxiv.org/search/?query={}&searchtype=all", query_encoded));
                    urls.push(format!("https://arxiv.org/search/?query={}&searchtype=title", query_encoded));
                    urls.push(format!("https://arxiv.org/search/?query={}&searchtype=abstract", query_encoded));
                    ...
                    urls.push(format!("https://api.semanticscholar.org/graph/v1/paper/search?query={}&limit=100", query_encoded));
                    
                    // ResearchGate (parcialmente gratis)
                    urls.push(format!("https://www.researchgate.net/search?q={}", query_encoded));
                    ...
                    urls.push(format!("https://api.openalex.org/works?search={}&per-page=100", query_encoded));
                    
                    // Unpaywall alternativas
                    urls.push(format!("https://unpaywall.org/api/search?query={}", query_encoded));
                }
                DeepWebSource::TechnicalDB => {
                    // ═══════════════════════════════════════════════════════════════
                    ...
                    urls.push(format!("https://link.springer.com/search?query={}", query_encoded));
                    
                    // PubMed (gratis)
                    urls.push(format!("https://pubmed.ncbi.nlm.nih.gov/?term={}", query_encoded));
                    
                    // Europe PMC (gratis)
                    urls.push(format!("https://europepmc.org/search?query={}", query_encoded));
                    
                    // CiteSeerX (gratis)
                    urls.push(format!("http://citeseerx.ist.psu.edu/search?q={}", query_encoded));
                ...
                DeepWebSource::CodeRepos => {
                    ...
                    urls.push(format!("https://gitee.com/search?type=repository&q={}", query_encoded));
                    
                    // Sourcehut
                    urls.push(format!("https://sr.ht/projects?search={}", query_encoded));
                    
                    // Launchpad
                    urls.push(format!("https://launchpad.net/+search?field.text={}", query_encoded));
                    
                    // GNU Savannah
                    urls.push(format!("https://savannah.gnu.org/search/?type_of_search=soft&words={}", query_encoded));
                }
                DeepWebSource::SpecializedForums => {
                    // ═══════════════════════════════════════════════════════════════
                    // 🔥 FOROS ESPECIALIZADOS
                    ...
                    // Reddit técnicos
                    for sub in &["programming", "rust", "python", "javascript", "reverseengineering", 
                                 "netsec", "hacking", "cybersecurity", "machinelearning", "datascience",
                                 "devops", "sysadmin", "linux", "opensource", "privacy"] {
                        urls.push(format!("https://www.reddit.com/r/{}/search/?q={}&restrict_sr=1&sort=relevance", sub, query_encoded));
                    }
                    ...
                    for site in &["security", "softwareengineering", "unix", "serverfault", "superuser"] {
                        urls.push(format!("https://{}.stackexchange.com/search?q={}", site, query_encoded));
                    }
                }
                DeepWebSource::DigitalLibraries => {
                    // ═══════════════════════════════════════════════════════════════
                    ...
                    urls.push(format!("https://archive.org/search.php?query={}", query_encoded));
                    urls.push(format!("https://archive.org/advancedsearch.php?q={}&output=json", query_encoded));
                    
                    // Project Gutenberg (ebooks gratis)
                    urls.push(format!("https://www.gutenberg.org/ebooks/search/?query={}", query_encoded));
                    
                    // Open Library
                    urls.push(format!("https://openlibrary.org/search?q={}", query_encoded));
                    
                    // Library Genesis mirrors (papers/libros, legal en muchos países)
                    urls.push(format!("https://libgen.is/search.php?req={}", query_encoded));
                    urls.push(format!("https://libgen.rs/search.php?req={}", query_encoded));
                    
                    // Z-Library mirrors
                    urls.push(format!("https://z-lib.org/s/{}", query_encoded));
                    
                    // Anna's Archive (metabúsqueda)
                    urls.push(format!("https://annas-archive.org/search?q={}", query_encoded));
                ...
                DeepWebSource::Archives => {
                    ...
                    // Scribd (vista previa)
                    urls.push(format!("https://www.scribd.com/search?query={}", query_encoded));
                    
                    // SlideShare
                    urls.push(format!("https://www.slideshare.net/search/slideshow?searchfrom=header&q={}", query_encoded));
                    
                    // Issuu
                    urls.push(format!("https://issuu.com/search?q={}", query_encoded));
                    
                    // DocDroid
                    urls.push(format!("https://www.docdroid.net/search?q={}", query_encoded));
                    
                    // Wayback Machine (versiones archivadas)
                    urls.push(format!("https://web.archive.org/cdx/search/cdx?url=*{}*&output=json&limit=100", query_encoded));
                    
                    // Common Crawl (datos web masivos)
                    urls.push(format!("https://index.commoncrawl.org/CC-MAIN-2023-50-index?url=*{}*&output=json", query_encoded));
                }
                DeepWebSource::Tor => {
                    // ═══════════════════════════════════════════════════════════════
                ...
                DeepWebSource::PremiumAPIs => {
                    ...
                    urls.push(format!("https://api.github.com/search/repositories?q={}&per_page=100", query_encoded));
                    urls.push(format!("https://api.github.com/search/code?q={}&per_page=100", query_encoded));
                    
                    // NPM Registry
                    urls.push(format!("https://registry.npmjs.org/-/v1/search?text={}&size=250", query_encoded));
                    
                    // Crates.io API
                    urls.push(format!("https://crates.io/api/v1/crates?q={}&per_page=100", query_encoded));
                    
                    // PyPI JSON API
                    urls.push(format!("https://pypi.org/pypi/{}/json", query_encoded));
                    
                    // RubyGems API
                    urls.push(format!("https://rubygems.org/api/v1/search.json?query={}", query_encoded));
                    
                    // Docker Hub
                    urls.push(format!("https://hub.docker.com/v2/search/repositories/?query={}&page_size=100", query_encoded));
                    
                    // Hugging Face API
                    urls.push(format!("https://huggingface.co/api/models?search={}&limit=100", query_encoded));
                    urls.push(format!("https://huggingface.co/api/datasets?search={}&limit=100", query_encoded));
                }
            }
        }
    ...
    fn detect_content_type(&self, html: &str, url: &str) -> String {
        ...
        if url_lower.contains("github") || url_lower.contains("gitlab") || url_lower.contains("code") {
            "code".to_string()
        } else if url_lower.contains("arxiv") || url_lower.contains("research") || url_lower.contains("paper") {
            "research".to_string()
    ...
    fn calculate_quality(&self, html: &str, url: &str) -> f32 {
        ...
        let url_lower = url.to_lowercase();
        if url_lower.contains("github") || url_lower.contains("gitlab") || url_lower.contains("arxiv") {
            score += 0.1;
        }
    ...
    fn extract_date(&self, html: &str) -> Option<String> {
        use regex::Regex;
        let re = Regex::new(r#"(?i)<meta[^>]*property=["']article:published_time["'][^>]*content=["']([^"']*)["']"#)
            .unwrap_or_else(|_| Regex::new("").unwrap());
    ...
    async fn find_access_methods(
            ...
            if let Ok(br) = bypass_result {
                if br.success {
                    methods.push(AccessMethod {
                        ...
                        instructions: format!("Contenido extraído ({} bytes). Usar API de Nuclear.", br.content.len()),
                        is_legal: true,
                        requires_auth: false,
                    });
                }
        ...
        // ═══════════════════════════════════════════════════════════════
        if url_lower.contains("ieee") || url_lower.contains("acm") || url_lower.contains("springer") 
            || url_lower.contains("nature.com") || url_lower.contains("sciencedirect") 
            || url_lower.contains("wiley") || url_lower.contains("tandfonline") {
            methods.push(AccessMethod {
        ...
        if result.is_premium || url_lower.contains("book") || url_lower.contains("ebook") {
            methods.push(AccessMethod {
                ...
                instructions: format!("https://libgen.is/search.php?req={}", urlencoding::encode(&result.title)),
                is_legal: true,
                requires_auth: false,
            });
            methods.push(AccessMethod {
                ...
                method_type: AccessMethodType::OpenRepository,
                instructions: format!("https://annas-archive.org/search?q={}", urlencoding::encode(&result.title)),
                is_legal: true,
                requires_auth: false,
            });
        }
        ...
        // ═══════════════════════════════════════════════════════════════
        if url_lower.contains("nytimes") || url_lower.contains("washingtonpost") 
            || url_lower.contains("wsj.com") || url_lower.contains("economist") 
            || url_lower.contains("medium.com") || url_lower.contains("bloomberg") {
            methods.push(AccessMethod {
        ...
        if url_lower.contains("github.com") {
            methods.push(AccessMethod {
                ...
                instructions: format!("Usar: curl https://api.github.com/repos/{}/{}", 
        ...
        methods.push(AccessMethod {
            ...
            method_type: AccessMethodType::AlternativeLegal,
            instructions: format!("https://webcache.googleusercontent.com/search?q=cache:{}", 
    ...
    fn extract_description(&self, html: &str) -> String {
        use regex::Regex;
        let re = Regex::new(r#"(?i)<meta[^>]*name=["']description["'][^>]*content=["']([^"']*)["']"#)
            .unwrap_or_else(|_| Regex::new("").unwrap());
        if let Some(cap) = re.captures(html) {
