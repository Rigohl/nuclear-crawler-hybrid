...
use nuclear_crawler_hybrid::ai::onlyfans_chatbot::{OnlyFansChatbot, ChatbotPersonality};
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Train { input, name, output } => {
...
fn scrape_html_fast(html: &str, source: &str) -> serde_json::Value {
    ...
    for line in lines.iter().take(100) { // Limitar para demo
...
fn train_bot(input: &str, name: &str, output: &str) -> Result<()> {
    ...
    // Cargar datos
    pipeline.load_scraped_data(&json_data)
