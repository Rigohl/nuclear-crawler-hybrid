...
# Insert just before the end of the `impl WebSearch` block
# We know `calculate_relevance` is the last method in `impl WebSearch` before `#[cfg(test)]`
insert_idx = content.find("    fn calculate_relevance(&self, query: &str, content: &str) -> f32 {")
if insert_idx == -1:
    print("Could not find calculate_relevance")
    sys.exit(1)

new_content = content[:insert_idx] + summary_func + "\n" + content[insert_idx:]

with open("src/core/web_search.rs", "w") as f:
    f.write(new_content)

print("Added generate_summary")
