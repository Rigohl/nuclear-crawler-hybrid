...
- **Integration test binary names**: `tests/integration_real_mcp.rs` references `nuclear_ultimate` and `src/bin/nuclear_ultimate.rs`, while `Cargo.toml` defines `nuclear-mcp` with path `src/bin/nuclear_mcp.rs`. The repository has **no `src/bin/` directory**, so these bin paths are mismatched and will make integration tests fail until reconciled.
