...
- **Integration test binary names**: `tests/integration_real_mcp.rs` references `nuclear_ultimate` and `src/bin/nuclear_ultimate.rs`, while `Cargo.toml` defines `nuclear-mcp` with path `src/bin/nuclear_mcp.rs` (that path does not exist). This mismatch will make integration tests fail until the bin paths are reconciled.
